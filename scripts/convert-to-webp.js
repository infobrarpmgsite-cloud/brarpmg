...
const sourceDir = path.join(__dirname, '../src/assets/images/properties');
...
async function convertImages(directory) {
    ...
    for (const entry of entries) {
        ...
        } else if (entry.name.endsWith('.avif') || entry.name.endsWith('.jpeg') || entry.name.endsWith('.jpg')) {
            const outputPath = fullPath.replace(/\.(avif|jpeg|jpg)$/, '.webp');
